...
dfpt <- function(y, A = NULL){
  ...
  if (any(is.na(y))) stop("Missing data (NA's) detected. Take actions (e.g., removing cases, removing features, imputation) to eliminate missing data and run dfpogst again.")
  if (!is.numeric(y))
  ...
  for (j in 1:d){
    eval_f1 = function(tt){
      return(ff = dpolikut(tt, y[,j], A))
    }
    optlik[j] <- nlminb(m[j], eval_f1,
                        lower = 0, upper = Inf,
                        control = list(trace = FALSE, rel.tol = 1e-6))$objective
    #optim(m[j], eval_f1, lower=0,upper=Inf, method = "L-BFGS-B")
  }
  lik1 <- sum(optlik)
  AIC1 <- 2/n*(lik1 + d)
  ...
  for (i in 1:(d-1)){
    for (j in (i+1):d){
      ...
      mtemp <- c(m[i], m[j])
      eval_f2 = function(tt){
        return(ff = dpolikgqt(tt, A, ytemp))
      }
      likmat[i,j]  <- nlminb(c(1e-3, mtemp), eval_f2,
                             lower = c(rep(1e-3,3)),
                             upper = c(rep(Inf,3)),
                             control = list(trace = FALSE, rel.tol = 1e-6))$objective
  ...
  while(stop == 0 && step < d)   {
    ...
    for (i in 1:(d1-1)){
      for (j in (i+1):d1){
        ...
        likmat[i,j]  <- nlminb(c(1e-3, mtemp), eval_f3,
                               lower = c(rep(1e-3,nvar)),
                               upper = c(rep(Inf,nvar)),
                               control = list(trace = FALSE, rel.tol = 1e-6))$objective
  ...
  while(stop == 0) {# indicates a non empty submodel
    ...
    if(nvar == 1){
      ftemp1 = function(tt1){
        return(ff = dpolikut(tt1, ytemp, A))
      }
      OPT <- nlminb(mtemp, ftemp1,
             lower = 1e-3, upper = Inf,
             control = list(trace = FALSE, rel.tol = 1e-6))
      estmu[1,j] <- OPT$par
    } else {
      nvar1 <- nvar+1
      ftemp2 <- function(tt){
      ...
      mtemp <- colMeans(ytemp)
      OPT2 <- nlminb(c(1e-3, mtemp), ftemp2,
                     lower = c(rep(1e-3,nvar1)),
                     upper = c(rep(Inf,nvar1)),
                     control = list(trace = FALSE, rel.tol = 1e-6))
      est <- OPT2$par
      estlambda[j] <- est[1]            # estimated lambda for jth submodel
      estmu[1:nvar, j] <- est[2:nvar1]  # estimated mu's for jth submodel
    } # end of else
...
#' @export
print.dfpt <- function(x, digits = 4, ...){
  cat("\nCall:\n", deparse(x$call), "\n\n", sep = "")
