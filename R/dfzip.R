#' @title Discrete factor analysis with the zero inflated Poisson distribution
#'
...
dfzip <- function(y){
  ...
  if (any(is.na(y))) stop("Missing data (NA's) detected. Take actions (e.g., removing cases, removing features, imputation) to eliminate missing data and run dfzips again.")
  if (!is.numeric(y))
  ...
  for (j in 1:d){
    eval_f1 = function(tt){
      return(ff = zippdf1(tt, y[,j]))
    }

    optlik[j] <- nlminb(c(0, m[j]), eval_f1,
                        lower = c(rep(0,3)),
                        upper = c(rep(1,1), rep(Inf,2)))$objective
  }
  lik1 <- sum(optlik)
  AIC1 <- (2/n) * (lik1 + 2*d)
  ...
  for (i in 1:(d-1)){
    for (j in (i+1):d){
      ...
      eval_f2 = function(tt){
        return(ff = dziplikg(tt, ytemp))
      }

      likmat[i,j] <- nlminb(c(rep(0,4), mtemp), eval_f2,
                            lower = c(rep(0,6)),
  ...
  while(stop == 0 && step < d)   {
    ...
    for (i in 1:(d1-1)){
      for (j in (i+1):d1){
        ...
        pars0 <- c(rep(0, nvar + 1), mtemp)
        likmat[i,j] <- nlminb(pars0, eval_f3,
                              lower = rep(0, length(pars0)),
                              upper = c(rep(1, nvar), rep(Inf, length(mtemp)+1)))$objective
  ...
  while(stop == 0) {                     # indicates a non empty submodel
    ...
    if(nvar == 1){
      ftemp1 = function(tt1){
        return(ff = zippdf1(tt1, ytemp))
      }

      OPT <- nlminb(c(0, m[j]), ftemp1,
                    lower = c(0,0),
                    upper = c(Inf,Inf))
      est <- OPT$par
      estpimu[1,j] <- est[1]
      ...
      OPT2 <- nlminb(c(rep(0, nvar+2), mtemp), ftemp2,
...
#' @export
print.dfzip <- function(x, digits = 4, ...){
  cat("\nCall:\n", deparse(x$call), "\n\n", sep = "")
